...
class Wpmmp_Settings {
	...
	function add_menu() {
		...
		$menu_title = __( 'Ninja Coming Soon', 'wpmp' );
	...
	function add_meta_boxes() {
		...
		do_action( 'wpmmp_add_metaboxes_settings' );
	...
	function settings_page() {
		...
		if ( isset( $_GET['tab'] ) )
			$tab = apply_filters( 'wpmmp_settings_get_tab', $_GET['tab'] );

		if ( ! isset( $_GET['tab'] ) )
			include wpmmp_settings_part( 'settings' );
		else
			include wpmmp_settings_part( $tab );
	...
	function save_settings() {
		...
		if ( $tab == 'settings' ) {
			...
			if ( ! defined( 'WPMMP_PRO_VERSION_ENABLED' ) && $theme == 'alissa' )
				$settings['theme'] = 'default-2';
		...
		do_action( 'wpmmp_save_settings', $tab );
	...
	function admin_tabs( $current = 'settings' ) {
	    ...
	    $tabs = apply_filters( 'wpmmp_settings_tabs', $tabs );
